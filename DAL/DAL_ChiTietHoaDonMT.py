...
def UpdateCTHoaDonMuonTra(maMuonTra, maSach,tenSach, tenTG, Gia, soLuong, ngayMuon, ngayTraDuKien, ngayTraThucTe, trangThai):
    conn = ConnectDB.connect_db()
    qr = "UPDATE ctmuontra SET maSach = %s, tenSach = %s, tenTG = %s, Gia = %s, soLuong = %s, ngayMuon = %s, ngayTraDuKien = %s, ngayTraThucTe = %s, trangThai = %s WHERE mamuontra = %s and maSach = %s" 
    data = (maSach, tenSach, tenTG, Gia, soLuong, ngayMuon, ngayTraDuKien, ngayTraThucTe, trangThai, maMuonTra, maSach)
    cs = conn.cursor()
    cs.execute(qr, data)
    conn.commit()
    conn.close()
...
def XuatFileCTHoaDon():
    ...
    ws.append(["mã hóa đơn", "Mã sách", "Tên sách", "Tên tác giả", "Giá", "Số lương", 
               "Ngày mượn", "Ngày trả dự kiến", "Ngày trả thực tế", "Trạng thái"])
    for row in rs:
        ws.append(row)

    wb.save('DanhSachChiTietMuontraAll.xlsx')